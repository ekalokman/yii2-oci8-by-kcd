...
- Enh: Custom side menu via `ekalokman\AdminOci8\Module::items` (mdmunir).
...
- Chg: Can customize return of `ekalokman\AdminOci8\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir).
...
- Chg: Remove `ekalokman\AdminOci8\components\BizRule` (mdmunir).
...
- Chg: Remove `ekalokman\AdminOci8\Module::allowActions`. Set access control directly with `ekalokman\AdminOci8\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `ekalokman\AdminOci8\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
